...
struct bst *bstInsert(struct bst *parent, int data){
    /* Write this function. */
    struct bst **insertLocation = &parent; /* parent is the current bst, and we want to traverse to the child node first */
    
	while(*insertLocation){ /* while the insert location is valid */
		if(data < (*insertLocation)->data){ /* if less, we go to left node */
			insertLocation = &((*insertLocation)->left); /* the address of the pointer to insertLocation's left node */
		} else { /* else go to right node */
			insertLocation = &((*insertLocation)->right);
		}
	}
    
    *insertLocation = (struct bst *) malloc(sizeof(struct bst)); /* make some space for the new node */
	assert(*insertLocation);
	
    /* give the node the value, then assign its child nodes as NULL */
	(*insertLocation)->left = NULL; /* init the child's children as NULL (or new tree with null pointers to the left/right nodes) */
	(*insertLocation)->right = NULL;
	(*insertLocation)->data = data; /* assign the data to the child node */
	
	return parent; /* return the initial *parent passed through WITH the added child in the correct location */
}


void freeTree(struct bst *parent){
    if(! parent){
        return; /* if theres no tree to be free, return nothing */
    }
    freeTree(parent->left); /* traverse recursively to the left node and free*/
    freeTree(parent->right); /* once all left nodes are done, go free the right nodes */
    free(parent); 
